...
function TopicList({ topics, setTopics, getTopicsCallback }: TopicListProps): React.ReactElement {
    ...
    const deleteTopicCallback = useCallback(async (
        ...
        if (status == 200) {
            const filteredTopics = topics.filter((t: TopicType) => shortName(t) !== topicName.name);
        ...
        <>
            {topics.length == 0 ? (
                ...
                <>
                    ...
                    <PublishMessage open={openPublishMessage} setOpen={setOpenPublishMessage} topicName={topicName} />
                    <TopicDefinition open={openTopicDefinition} setOpen={setOpenTopicDefinition} topicName={topicName} />
                </>
            )}
        </>
    )
